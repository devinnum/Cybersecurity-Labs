...
def generate_prime(bits, k):
	...
	while True:
		...
		n = random.randint(1,2**(bits+1)-1) + 2
		if n % 2 == 0:
			n += 1
		count += 1
...
def generateKeys(bitlength):
	...
	e = n + 1
	while e > n:
		e = generate_prime(bitlength * 2, 20)
...
def encrypt(key, plaintext):
	...
	bigint = convertFromASCII(plaintext)

	strint = str(bigint)
	
	for i in range((len(strint) // 2) + 1):
		p.append(strint[i * 2 : (i * 2 + 2)])
	

	for i in p:
		c.append(pow(int(i), key.e, key.n))
	
	return ciphertext(c, len(plaintext), 2)
...
def decrypt(key, cipherText):
	p = []
	index = 0
	length = len(cipherText.c)
	for i in cipherText.c:
		if (len(str(pow(i, key.d, key.n))) == 1) and (index < (length - 1)):
			p.append("0" + str(pow(i, key.d, key.n)))
		else:
			p.append(str(pow(i, key.d, key.n)))
		index += 1
	bigint = ""
	for i in p:
		bigint = bigint + i
	return convertToASCII(int(bigint))
...
# object.
def crack(key):
	p = generate_prime(key.l, 20)
	q = generate_prime(key.l, 20)
	while (p * q != key.n):
		p = generate_prime(key.l, 20)
		q = generate_prime(key.l, 20)
	pq = (p - 1) * (q - 1)
...
def sign(key, plaintext):
	c = []
	p = []
	hash = str(int(hashlib.sha256(bytes(plaintext,'ascii')).hexdigest(), 16))
	
	for i in range((len(hash) // 2) + 1):
		p.append(hash[i * 2 : (i * 2 + 2)])
	
	for i in p:
		c.append(pow(int(i), key.d, key.n))
...
def checkSign(key,plaintext,signature):
	p = []
	index = 0
	length = len(signature.c)
	for i in signature.c:
		if (len(str(pow(i, key.e, key.n))) == 1) and (index < (length - 1)):
			p.append("0" + str(pow(i, key.e, key.n)))
		else:
			p.append(str(pow(i, key.e, key.n)))
		index += 1
	hash = ""
	for i in p:
		hash = hash + i

	plainhash = int(hashlib.sha256(bytes(plaintext,'ascii')).hexdigest(), 16)

	if int(hash) == int(plainhash):
		return True
	else:
		return False
